...
def train_model(model, criterion, optimizer, num_epochs=25, phase='train', test_list = test_data_list, train_list = train_list):
    ...
    torch.save(model.state_dict(), '/data/padmalab/ecg/data/processed/fl_dl_temp/dp_1_resnet_plots_40_model.pt')
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.savefig('dp_1_resnet_plots_40_loss.jpg')
    plt.clf()
    ...
    plt.xlabel('Epoch')
    plt.ylabel('AUROC')
    plt.savefig('dp_1_resnet_plots_40_auroc.jpg')
    # print(eloss_graph)
    return model
...
#         correct = np.squeeze(correct_tensor.numpy()) if not torch.cuda.is_available() else np.squeeze(correct_tensor.cpu().numpy())
    ...
    with open('/data/padmalab/ecg/data/processed/fl_dl_temp/dp_1_resnet_plots_40.pickle', 'wb') as file:
