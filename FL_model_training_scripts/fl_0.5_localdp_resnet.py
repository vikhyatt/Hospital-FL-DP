...
class ClientUpdate(object):
    ...
    def train(self, model):
        ...
        early_stopping = EarlyStopping(patience=10, verbose=True, path =  "FL_DP0.5_models/fl_client%s_resnet_weights.pt"%self.client)
        ...
        print("final_epsilon value:",privacy_engine.get_epsilon(1e-5))
        e_value = privacy_engine.get_epsilon(1e-5)
        total_loss = sum(e_loss)/len(e_loss)
        model.load_state_dict(torch.load("FL_DP0.5_models/fl_client%s_resnet_weights.pt"%self.client))
        return model.state_dict(), total_loss,e_value
...
def training(model, rounds, batch_size, lr, ds, agsx, train_data_dict, val_data_dict, S_t, E, plt_title, plt_color):
    ...
    for curr_round in range(1, rounds+1):
        ...
        for k in S_t[curr_round-1]:
            ...
            weights, loss, e_val = local_update.train(model=copy.deepcopy(model))

            w.append(copy.deepcopy(weights))
            local_loss.append(copy.deepcopy(loss))
            epsilon_values.append(e_val)
...
#     print("Total time taken to Train: {}".format(end-start))
    ...
    torch.save(model.state_dict(), 'FL_DP0.5_models/fl_resnet_localdp_0.5_weights.pth')
    np.save('FL_DP0.5_models/fl_resnet_localdp_0.5_evals.npy',np.array(epsilon_values))
    return model
...
resnet_trained.eval()
resnet_trained.load_state_dict(torch.load("FL_DP0.5_models/fl_resnet_localdp_0.5_weights.pth"))
