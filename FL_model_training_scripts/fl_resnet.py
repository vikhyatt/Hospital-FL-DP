...
class ClientUpdate(object):
    ...
    def train(self, model):
        ...
        early_stopping = EarlyStopping(patience=10, verbose=True, path =  "models/fl_client%s_resnet_weights.pt"%self.client)
        ...
        model.load_state_dict(torch.load("models/fl_client%s_resnet_weights.pt"%self.client))
...
#     print("Total time taken to Train: {}".format(end-start))
    ...
    torch.save(model.state_dict(), 'models/fl_resnet_weights.pth')
    return model
...
#         correct = np.squeeze(correct_tensor.numpy()) if not torch.cuda.is_available() else np.squeeze(correct_tensor.cpu().numpy())
    ...
    # with open('/data/padmalab/ecg/data/processed/fl_dl_temp/fl_resnet_pred_dict.pickle', 'wb') as file:
...
resnet_trained.load_state_dict(torch.load("models/fl_resnet_weights.pth"))
